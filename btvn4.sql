...
BEGIN
    IF p_id IS NULL AND p_phone IS NULL THEN
        SET p_amount = NULL;
        SET p_status = 'Lỗi: Vui lòng nhập ID hoặc Số điện thoại';

    ELSEIF p_id IS NOT NULL THEN
        SELECT pi.total_due
        INTO   p_amount
        FROM   Patient_Invoices pi
        WHERE  pi.patient_id = p_id
        LIMIT  1;

        IF p_amount IS NULL THEN          
            SET p_amount = 0;
            SET p_status = 'Không tìm thấy bệnh nhân với ID đã nhập';
        ELSE
            SET p_status = 'Tra cứu thành công theo ID';
        END IF;
    ...
    ELSE
        ...
        IF p_amount IS NULL THEN      
            SET p_amount = 0;
            SET p_status = 'Không tìm thấy bệnh nhân với SĐT đã nhập';
        ELSE
            SET p_status = 'Tra cứu thành công theo Số điện thoại';
        END IF;
    END IF;
END //
